...
export class ApiClientService {
  api = 'https://myfirstproject-4444.de.r.appspot.com/'
  ...
  getEvents(eventsRequest: EventsRequest): Observable<EventsResponse> {
    ...
    return this.http.get<EventsResponse>(this.api + '/events?', options);
  }

  getOptions(keyword: string): Observable<string[]> {
    const httpParams = new HttpParams().set('keyword', keyword);

    return this.http.get<string[]>(this.api + '/suggestions', { params: httpParams });
  }

  getEventDetails(id: string): Observable<EventDetails> {
    return this.http.get<EventDetails>(this.api + '/event/' + id);
  }
  ...
  getArtistDetails(artists: Artist[]): ArtistDetails[] {
    ...
    artists.forEach((artist) => {
      if (artist.musicRelated) {
        ...
        this.http
          .get<ArtistDetails>(this.api + '/artist?', { params: httpParams })
  ...
  getVenueDetails(venueName: string): Observable<VenueDetails> {
    ...
    return this.http.get<VenueDetails>(this.api + '/venue?', { params: httpParams });
  }
  ...
  getUserLocation(): Observable<coordinates> {
    return this.http
      .get<coordinates>('https://ipinfo.io/json?token=####')
      .pipe(
        map((data) => {
